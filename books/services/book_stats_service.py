...
class BookStatsService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        authors_data = [{'author': item['author'], 'count': item['book_count']} 
    ...
    def get_publication_year_distribution(self, books=None) -> List[Dict[str, Any]]:
        ...
        return [{'year': k, 'count': v} for k, v in 
               sorted(year_counts.items(), key=lambda item: item[0], reverse=True)]
    ...
    def get_top_authors(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        return [{"author": author, "book_count": count} for author, count in sorted_authors]
    ...
    def get_recently_added_count(self, days: int = 30) -> int:
        ...
        return sum(1 for book in books if hasattr(book, 'created_at') and 
                  book.created_at and book.created_at >= threshold_date)
