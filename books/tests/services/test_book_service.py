...
class BookServiceTestCase(TestCase):
    ...
    def test_create_book_with_multiple_isbns(self):
        ...
        isbns = [
            unique_isbn,
            "9780747532699",
        ]  # Valid ISBN-13 for Harry Potter and the Philosopher's Stone
        book_data = {
            "title": "New Book",
            "isbn": unique_isbn,
            "published_date": "2023-01-01",
        ...
        self.assertTrue(
            BookISBN.objects.filter(book=book, isbn="9780747532699").exists()
        )
    ...
    def test_get_book_by_all_isbns(self):
        ...
        BookISBN.objects.get_or_create(
            book=book, isbn=primary_isbn, defaults={"type": "ISBN-13"}
        )
        BookISBN.objects.get_or_create(
            book=book, isbn=secondary_isbn, defaults={"type": "ISBN-13"}
        )
    ...
    def test_enrich_book_by_isbn_update(self):
        ...
        updated_book = self.book_service.enrich_book_data(
            existing_book, mock_enrichment_service
        )

        # Assert: Check if the book was updated with enriched data
        self.assertEqual(
            updated_book.title, "Updated Harry Potter and the Philosopher's Stone"
        )
        # Check if author is in the list of authors (since enrich might return a list of strings)
        authors = (
            updated_book.authors.all()
            if hasattr(updated_book.authors, "all")
            else updated_book.authors
        )
        # Check if author is in the list of authors (since enrich might return a string)
        author_found = any(
            (hasattr(author, "name") and author.name == "J.K. Rowling")
            or (isinstance(author, str) and author == "J.K. Rowling")
