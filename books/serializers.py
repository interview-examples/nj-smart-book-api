...
class EnrichedBookSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Book
        fields = ['id', 'title', 'authors', 'isbn', 'description', 'published_date', 'enriched_data']
    ...
    def get_enriched_data(self, obj: Book) -> Dict[str, Any]:
        ...
        if enriched:
            return {
                ...
                'publication_year': enriched.published_date if enriched.published_date else None,
...
class BookCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Book
        fields = ['title', 'authors', 'isbn', 'description', 'published_date', 'auto_fill']
            ...
            try:
                ...
                for fmt in ("%d.%m.%Y", "%m/%d/%Y", "%Y/%m/%d"):
                    try:
                        ...
                        continue
                raise serializers.ValidationError("Invalid date format. Use YYYY-MM-DD, YYYY, YYYY-MM, DD.MM.YYYY, or MM/DD/YYYY.")
            except Exception:
                raise serializers.ValidationError("Invalid date format. Use YYYY-MM-DD, YYYY, YYYY-MM, DD.MM.YYYY, or MM/DD/YYYY.")
